...
func TestWithScripts(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, "Invalid script name: \"InvalidScript\". Scripts must have a name on the form: <STATE_NAME>_<ACTION>_<ORDERING_NUMBER>_<OPTIONAL_DESCRIPTION>. For example: 'ArtifactInstall_Enter_05_wifi-driver' is a valid script name.\n",
		fakeErrWriter.String())
}
